...
\begin{args}
	\arg hms (TEXT) -- hms to be converted. The following syntaxes must be accepted: 'h:m:s', 'hhmmss' and 'hHmMsS'. Hours and minutes should be integer and seconds can be decimal. 
\end{args}
...
This function converts dms (degrees, minutes, seconds) to degrees format. This is useful for declination conversion (DEC) conversion, since dms is fairly standard method of presenting DEC coordinates.

\begin{description}
\item[Parameters]

\begin{args}
	\arg dms (TEXT) -- dms to be converted. The following syntaxes must be accepted: 'd:m:s', 'ddmmss' and 'dDmMsS'. Degrees and minutes should be integer and seconds can be decimal.
\end{args}
